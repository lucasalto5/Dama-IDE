import { ClipboardEvent as ReactClipboardEvent, Component, createElement, CSSProperties, DragEvent as ReactDragEvent, ErrorInfo, FormEvent, KeyboardEvent as ReactKeyboardEvent, ReactNode, useEffect, useLayoutEffect, useMemo, useRef, useState } from "react";
...
function SettingsCenter({ initial, modelsState, onModelsChange, onClose, onSave, onConfigureModel, onResetOnboarding }: { initial: DamaSettings; modelsState: ModelsState; onModelsChange: (state: ModelsState) => void; onClose: () => void; onSave: (settings: DamaSettings) => Promise<void>; onConfigureModel: () => void; onResetOnboarding: () => Promise<void> }) {
  ...
  const [engineInstallProgress, setEngineInstallProgress] = useState<DamaEngineInstallProgress | null>(null);
  ...
  useEffect(() => {
    ...
    const dispose = window.dama?.onDamaQuota?.((quota) => { if (current) setEngineStatus((status) => status ? { ...status, quota } : status); });
    return () => { current = false; dispose?.(); };
  }, []);

  useEffect(() => window.dama?.onDamaEngineInstallProgress?.((progress) => {
    setEngineInstallProgress(progress);
    if (progress.error) setEngineError(progress.error);
  }), []);
  ...
  async function runEngineAction(action: "install" | "verify" | "remove") {
    ...
    setEngineBusy(true);
    setEngineError(null);
    if (action === "install") setEngineInstallProgress({ status: "checking", percent: 0, detail: "Consultando o pacote oficial…", error: null });
    try {
      ...
      if (action === "install" || action === "remove") onModelsChange(await window.dama.listModels());
    } catch (error) {
  ...
  return <div className="settings-backdrop"><section className="settings-center">
    <aside><div className="settings-brand"><DinoLogo /><strong>Configurações</strong></div><nav>{sections.map((item) => { const Icon = item.icon; return <button key={item.id} className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><Icon size={14} />{item.label}</button>; })}</nav><div className="settings-version">Dama 0.12.3 · preview</div></aside>
    <main><header><div><span className="eyebrow">Preferências</span><h2>{sections.find((item) => item.id === section)?.label}</h2></div><button className="icon-button" onClick={onClose}><X size={17} /></button></header><div className="settings-scroll" key={section}>
      ...
      {section === "dama" && <>
        <SettingsGroup title="Motor local da Dama" description="Componente opcional baseado no Carnaval IA. O agente e as ferramentas rodam neste computador; as respostas continuam usando os modelos configurados por você.">
          ...
          {engineStatus?.installed && engineStatus.quota && <div className={`dama-quota-card ${engineStatus.quota.exhausted ? "exhausted" : ""}`}>
            <div className="dama-quota-ring" style={{ "--quota": engineStatus.quota.percentage } as CSSProperties}><span><strong>{engineStatus.quota.percentage}%</strong><small>restante</small></span></div>
            <div className="dama-quota-copy"><span className="eyebrow">Limite do Dama AI</span><strong>{engineStatus.quota.exhausted ? "Limite encerrado" : `${engineStatus.quota.remaining.toLocaleString("pt-BR")} tokens disponíveis`}</strong><p>{engineStatus.quota.used.toLocaleString("pt-BR")} de {engineStatus.quota.limit.toLocaleString("pt-BR")} tokens utilizados neste ciclo.</p><small>Renova em {new Date(engineStatus.quota.renewsAt).toLocaleDateString("pt-BR")}. O consumo é registrado somente após uma resposta confirmada.</small></div>
          </div>}
          ...
          {engineBusy && engineInstallProgress && <div className="dama-engine-download"><div><span>{engineInstallProgress.detail}</span><small>{engineInstallProgress.status === "downloading" ? `${Math.round(engineInstallProgress.percent)}%` : ""}</small></div><div className="update-progress"><span style={{ width: `${engineInstallProgress.status === "checking" ? 8 : Math.max(8, engineInstallProgress.percent)}%` }} /></div><small>O arquivo é verificado antes de ser ativado.</small></div>}
          {engineError && <div className="settings-inline-warning"><AlertCircle size={14} /><span>{engineError}</span></div>}
          <div className="dama-engine-actions">{!engineStatus?.installed ? <button className="primary-button" disabled={engineBusy} onClick={() => void runEngineAction("install")}>{engineBusy ? <LoaderCircle className="spin" size={13} /> : <Sparkles size={13} />} {engineBusy ? "Instalando…" : "Instalar Dama AI"}</button> : <button className="secondary-button" disabled={engineBusy} onClick={() => void runEngineAction("verify")}>{engineBusy ? <LoaderCircle className="spin" size={13} /> : <ShieldCheck size={13} />} Verificar arquivos</button>}</div>
        </SettingsGroup>
...
function PostUpdateScreen({ state, onContinue, onRollback }: { state: UpdateState; onContinue: () => void; onRollback: () => void }) {
  const update = state.postUpdate!;
  const notes = String(update.releaseNotes || "Melhorias de estabilidade, segurança e experiência da Dama.").split(/\r?\n/).map((line) => line.replace(/^\s*[-#*]+\s*/, "").trim()).filter(Boolean);
  const rollbackBusy = state.rollbackStatus === "checking" || state.rollbackStatus === "downloading";
  return <div className="update-screen post-update-screen"><div className="update-shell"><DinoLogo animated /><span className="eyebrow">Dama {update.version}</span><h1>Atualização concluída</h1><p>A Dama foi atualizada e seus projetos, conversas, configurações e componentes locais foram preservados.</p><section className="update-notes"><strong>O que mudou</strong>{notes.slice(0, 10).map((note, index) => <span key={`${index}-${note}`}><Check size={11} />{note}</span>)}</section>{rollbackBusy && <><div className="update-progress"><span style={{ width: `${Math.max(3, state.rollbackPercent)}%` }} /></div><div className="update-progress-meta"><span>{state.rollbackStatus === "checking" ? "Procurando versão anterior…" : `Baixando versão anterior · ${Math.round(state.rollbackPercent)}%`}</span></div></>}{state.rollbackError && <div className="settings-inline-warning"><AlertCircle size={14} /><span>{state.rollbackError}</span></div>}<div className="update-actions"><button className="quiet-button" disabled={rollbackBusy} onClick={onRollback}><Undo2 size={13} /> Restaurar versão anterior</button><button className="primary-button" disabled={rollbackBusy} onClick={onContinue}>Continuar</button></div></div></div>;
}
...
export default function App() {
  ...
  if (startupReady && updateState?.postUpdate) return <PostUpdateScreen state={updateState} onContinue={() => void window.dama?.acknowledgeUpdate().then(setUpdateState)} onRollback={() => void window.dama?.rollbackUpdate().then(setUpdateState)} />;
