...
function SettingsCenter({ initial, modelsState, onModelsChange, onClose, onSave, onConfigureModel, onResetOnboarding }: { initial: DamaSettings; modelsState: ModelsState; onModelsChange: (state: ModelsState) => void; onClose: () => void; onSave: (settings: DamaSettings) => Promise<void>; onConfigureModel: () => void; onResetOnboarding: () => Promise<void> }) {
  ...
  return <div className="settings-backdrop"><section className="settings-center">
    <aside><div className="settings-brand"><DinoLogo /><strong>Configurações</strong></div><nav>{sections.map((item) => { const Icon = item.icon; return <button key={item.id} className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><Icon size={14} />{item.label}</button>; })}</nav><div className="settings-version">Dama 0.12.2 · preview</div></aside>
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (window.dama) {
        ...
        } else if (window.dama.apiVersion !== 11) {
        ...
        if (window.dama.apiVersion !== 11) setError("A Dama foi atualizada. Feche o aplicativo completamente e abra novamente para carregar o limite do Dama AI e o histórico de atualizações.");
      }
